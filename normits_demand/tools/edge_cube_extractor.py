...
def MAT2OMX(exe_cube: str, matFile: str, outPath: str, outFile: str):
    ...
    to_write = [
        f'convertmat from="{matFile}" to="{outPath}\\{outFile}.omx" format=omx compression=4'
    ]
    ...
    proc_single(
        [
            f'"{exe_cube}" "{outPath}\\Mat2OMX.s" -Pvdmi /Start /Hide /HideScript',
            f'del "{outPath}\\*.prn"',
            f'del "{outPath}\\*.VAR"',
            f'del "{outPath}\\*.PRJ"',
...
def StnZone2StnTLC(stnZone2Node: str, railNodes: str, extNodes: str, overwrite_TLCs):
    ...
    # keep needed columns
    extNodes = extNodes[
        ["N", "X", "Y", "STATIONCODE", "STATIONNAME", "ZONEID", "TFN_FLAG", "Category_ID"]
    ]
...
def PTDemandFromTo(exe_cube: str, cat_folder: str, run_folder: str, output_folder: str):
    ...
    to_write = [
        ...
        f"MW[17]=MI.1.19 ;209",
        f"MW[18]=MI.1.17 ;207",
        f"MW[19]=MI.1.18 ;208" f";FILLMW MW[21]=MI.1.1.T(10) ; PA to home",
