            ...
            for pp in dctpurp:
                for tp in dcttp:
                    ...
                    zone_te = pd.merge(zone_te,
                                       ...
                                       on=['zone'])
                    #Export as csv
                    zone_te.to_csv(f'Y:/Mobile Data/Processing/MDD_Check_HW/zone_te_od_p{pp}_m{md}_tp{tp}.csv', 
                                   index = False)
                    #Master list
                    zone_te['mode'] = md
                    zone_te['weekday'] = wd
                    ...
                    mat.plot(x='nmtrip', y='linearfit', color='Red', ax=ax,
                                  xlabel='NoHAM trips',
                                  ylabel='MDD trips')
                    ax.figure.savefig(f'Y:/Mobile Data/Processing/MDD_Check_HW/zone_od_p{pp}_m{md}_tp{tp}.png')
...
master_te.to_csv('Y:/Mobile Data/Processing/MDD_Check_HW/trip_ends_zone.csv', 
                                   index = False)
#Concat and export LAD trip ends
master_lad_te = pd.concat(lad_te_list)
master_lad_te.to_csv('Y:/Mobile Data/Processing/MDD_Check_HW/trip_ends_lad.csv', 
                                   index = False)               
#Concat and export LAD-LAD matrix
master_lad = pd.concat(lad_list)
master_lad.to_csv('Y:/Mobile Data/Processing/MDD_Check_HW/lad-lad_matrices.csv', 
                                   index = False)
#Concat and export Sector trip ends
master_sec_te = pd.concat(sec_te_list)
master_sec_te.to_csv('Y:/Mobile Data/Processing/MDD_Check_HW/trip_ends_sectors.csv', 
                                   index = False)
#Concat and export sector-Sector matrix
master_sec = pd.concat(sec_list)
master_sec.to_csv('Y:/Mobile Data/Processing/MDD_Check_HW/sector-sector_matrices.csv', 
                                   index = False)
#Concat and export TLD
master_tld = pd.concat(dband_list)
master_tld.to_csv('Y:/Mobile Data/Processing/MDD_Check_HW/tld.csv', 
                                   index = False)

# save as excel file
with pd.ExcelWriter('Y:/Mobile Data/Processing/MDD_Check_HW/MDD-NoHAM_Summary.xlsx') as writer:
