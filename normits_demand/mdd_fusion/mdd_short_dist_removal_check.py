...
def te_ans(mat,
    ...
    ax = zone_te.plot.scatter(y='d_mdd', x='d_nm',
                              title='Regression: y = ' + str(d[0].round(2)) + 'x + ' + str(
                                  d[0].round(1)) +
                                    ', R^2 = ' + str(r2.round(2)))
    ...
    for wd in dctday:
        for pp in dctpurp:
            for tp in dcttp:
                ...
                # Produce x10km trip plot
                mat = mat[mat['dband'] >= 11]
                # Produce trip plots
                scen = 'x10km'
                # zone-zone linear regression
                #zz_reg(mat, pp, md, tp, scen)
