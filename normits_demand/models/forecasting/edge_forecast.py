...
def AddTLC2StationsMatrix(mx, stnTLC):
    ...
    #keep needed columns
    mx = mx[['from_model_zone_id', 'to_model_zone_id', 'from_stn_zone_id', 'to_stn_zone_id',
             'userclass', 'Distance', 'Demand', 'STATIONCODE', 'STATIONNAME']]
    #rename column
    ...
    #keep needed columns
    mx = mx[['from_model_zone_id', 'to_model_zone_id', 'from_stn_zone_id', 'O_TLC',
             'O_StnName', 'to_stn_zone_id', 'STATIONCODE', 'STATIONNAME', 'userclass',
             'Distance', 'Demand']]
    #rename column
...
def Preparestn2stnMatrix(demandMX, iRSjProps, distMX, stnTLC):
    ...
    df = demandMX.merge(iRSjProps, how='left', on=['from_model_zone_id', 'to_model_zone_id', 'userclass'])
    #fill nans if any
    #df = df.fillna(0)
    #calculate movement demand proportion
    df['Demand'] = df['Demand'] * df['proportion']
    #group by stn2stn
    df = df.groupby(['from_model_zone_id', 'to_model_zone_id', 'from_stn_zone_id',
                     'to_stn_zone_id', 'userclass'])['Demand'].sum().reset_index()
    ...
    #keep needed columns
    df = df[['from_model_zone_id', 'to_model_zone_id', 'from_stn_zone_id',
             'to_stn_zone_id', 'userclass', 'tran_distance', 'Demand']]
    #rename column
...
def Preparestn2stnMatrixToHome(demandMX, iRSjProps, distMX, stnTLC):
    ...
    #merge demand matrix to iRSj probabilities
    df = demandMX.merge(iRSjProps, how='left', left_on=['from_model_zone_id', 'to_model_zone_id', 'userclass'],
                        right_on=['to_model_zone_id', 'from_model_zone_id', 'userclass'])
    #rename column
    df = df.rename(columns={'from_model_zone_id_x':'from_model_zone_id',
...
def AssignEDGEFlow(flows_file, flows_lookup, mx):
    ...
    '''
    #rename column
    edge_flows = flows_file.rename(columns={'FromCaseZoneID':'O_TLC', 'ToCaseZoneID':'D_TLC'})
    #keep needed columns
...
def AddDistanceBandTAGFlow(df):
    ...
    #Outside South East
    df['TAG_Flow'].loc[(df['TAG_NonDist'] == 'Outside South East') & (df['Distance'] < 25)] = 'Outside South East <25 miles'
    df['TAG_Flow'].loc[(df['TAG_NonDist'] == 'Outside South East') & (df['Distance'] >= 25) & (df['Distance'] < 100)] = 'Outside South East 25 to 100 miles'
    df['TAG_Flow'].loc[(df['TAG_NonDist'] == 'Outside South East') & (df['Distance'] >= 100)] = 'Outside South East  100 + miles - adjusted'
    #Outside South East to/from London
    df['TAG_Flow'].loc[(df['TAG_NonDist'] == 'Outside South East to/from London') & (df['Distance'] < 100)] = 'Outside South East to/from London < 100 miles'
    df['TAG_Flow'].loc[(df['TAG_NonDist'] == 'Outside South East to/from London') & (df['Distance'] >= 100)] = 'Outside South East to/from London 100 + miles'
...
def ApplyTicketSplits(df):
    ...
    #keep needed columns
    df = df[['from_model_zone_id', 'to_model_zone_id', 'from_stn_zone_id', 'O_TLC',
             'to_stn_zone_id', 'D_TLC', 'userclass',
             'Purpose', 'F', 'R', 'S']]
...
def CreateFactorsForMissingMOIRAMovements(mx, edgeFactors, other_tickets_df, no_factors_df):
    ...
    #keep needed columns
    edgeFactors = edgeFactors[['ZoneCodeFrom', 'ZoneCodeTo', 'purpose', 'TicketType',
                                'Demand_rate', 'Flag']]
    ...
    #melt Matrix
    mx = pd.melt(mx, id_vars=['from_model_zone_id', 'to_model_zone_id', 'from_stn_zone_id',
                              'O_TLC', 'to_stn_zone_id', 'D_TLC', 'userclass',
                              'Purpose'], value_vars=['F', 'R', 'S'])
    #rename column
    mx = mx.rename(columns={'value': 'T_Demand', 'variable':'TicketType', 
                            'O_TLC':'ZoneCodeFrom', 'D_TLC':'ZoneCodeTo',
                            'Purpose': 'purpose'})
    #merge factors to matrix
    mx = mx.merge(edgeFactors, how='left', on=['ZoneCodeFrom', 'ZoneCodeTo', 'TicketType', 'purpose'])    
    ...
    missing_moira_zonal['Internal'][(missing_moira_zonal['from_model_zone_id'] < 1158) & (missing_moira_zonal['to_model_zone_id'] < 1158)]
    #group to stn2stn
    missing_moira = missing_moira.groupby(['ZoneCodeFrom', 'ZoneCodeTo', 'TicketType', 'purpose'])['T_Demand'].sum().reset_index()

    #merge factors
    missing_moira = missing_moira.merge(edgeFactors, how='left', on=['ZoneCodeFrom', 'ZoneCodeTo', 'purpose'])
    missing_moira_zonal = missing_moira_zonal.merge(edgeFactors, how='left', on=['ZoneCodeFrom', 'ZoneCodeTo', 'purpose'])
    #rename column
    missing_moira = missing_moira.rename(columns={'TicketType_y': 'Available_TicketType',
                                                  'TicketType_x':'Missing_TicketType'})
    missing_moira_zonal = missing_moira_zonal.rename(columns={'TicketType_y': 'Available_TicketType',
                                                  'TicketType_x':'Missing_TicketType'})
    #keep those that have available records
    available_ticket = missing_moira[~missing_moira['Available_TicketType'].isna()].reset_index(drop=True)
        #keep one ticket type
    available_ticket = available_ticket.drop_duplicates(subset=['ZoneCodeFrom', 'ZoneCodeTo', 'Missing_TicketType', 'purpose'])
    #keep needed columns
    available_ticket = available_ticket[['ZoneCodeFrom', 'ZoneCodeTo', 'purpose',
                                     'Missing_TicketType', 'Available_TicketType', 'Demand_rate']]
    
    #keep needed columns
    available_ticket = available_ticket[['ZoneCodeFrom', 'ZoneCodeTo', 'purpose',
                                         'Missing_TicketType', 'Demand_rate']]
    #rename columns
    ...
    #logging DFs
    #keep those that have available records
    available_ticket_log = missing_moira_zonal[~missing_moira_zonal['Available_TicketType'].isna()].reset_index(drop=True)
    available_ticket_log = available_ticket_log.groupby(['ZoneCodeFrom', 
                                                         'ZoneCodeTo', 'purpose',
                                                         'Missing_TicketType', 'Available_TicketType', 'Internal'])['T_Demand'].sum().reset_index()
    ...
    missing_ticket = missing_moira_zonal[missing_moira_zonal['Available_TicketType'].isna()].reset_index(drop=True)
    #keep needed columns
    missing_ticket = missing_ticket.groupby(['ZoneCodeFrom', 'ZoneCodeTo', 'Internal'])['T_Demand'].sum().reset_index()
    #log these movments to main dataframe
...
def ApplyEDGEGrowthMethod1From(mx, edgeFactors):
    ...
    #melt Matrix
    mx = pd.melt(mx, id_vars=['from_model_zone_id', 'to_model_zone_id', 'from_stn_zone_id',
                              'O_TLC', 'to_stn_zone_id', 'D_TLC', 'userclass','Purpose'],
                 value_vars=['F', 'R', 'S'])
    #rename column
    mx = mx.rename(columns={'value': 'T_Demand', 'variable':'TicketType', 
                            'O_TLC':'ZoneCodeFrom', 'D_TLC':'ZoneCodeTo',
    ...
    #keep needed columns
    mx = mx[['from_model_zone_id', 'to_model_zone_id', 'from_stn_zone_id', 'ZoneCodeFrom',
             'to_stn_zone_id', 'ZoneCodeTo', 'userclass', 'purpose', 'TicketType',
             'T_Demand', 'N_Demand']]
...
def ApplyEDGEGrowthMethod1To(mx, edgeFactors):
    ...
    mx = mx.rename(columns={'value': 'T_Demand', 'variable':'TicketType', 
                            ...
                            'Purpose': 'purpose'})
    #merge new factors file to matrix
    mx = mx.merge(edgeFactors, how='left', left_on=['ZoneCodeFrom', 'ZoneCodeTo', 'TicketType', 'purpose'],
    ...
    mx['N_Demand'] = mx['T_Demand'] * mx['Demand_rate']
    #keep needed columns
    mx = mx[['from_model_zone_id', 'to_model_zone_id', 'from_stn_zone_id', 'ZoneCodeFrom_x',
             'to_stn_zone_id', 'ZoneCodeTo_x', 'userclass', 'purpose', 'TicketType',
             'T_Demand', 'N_Demand']]
    #rename column
...
def Prepare_logging_info(other_tickets_df, no_factors_df, demand_total): 
    ...
    other_tickets_df = other_tickets_df.groupby(['ZoneCodeFrom', 'ZoneCodeTo', 'purpose',
                                                 'Missing_TicketType', 'Available_TicketType', 'Internal'])['T_Demand'].sum().reset_index()
    ...
    demand_total_ticket_internal = other_tickets_df['T_Demand'][other_tickets_df['Internal'] == 1].sum()
    #movements with no factor at all
    no_factors_df = no_factors_df.groupby(['ZoneCodeFrom', 'ZoneCodeTo', 'Internal'])['T_Demand'].sum().reset_index()
    #get demand totals for movements where different ticket type was used
    demand_total_factors = no_factors_df['T_Demand'].sum()
    #demand total for internals
    demand_total_factors_internal = no_factors_df['T_Demand'][no_factors_df['Internal'] == 1].sum()
    ...
    factors_internal_prop = round(demand_total_factors_internal / demand_total_factors* 100,3)
    #total proportions
    ...
    other_tickets_df = other_tickets_df.groupby(['ZoneCodeFrom', 'ZoneCodeTo', 'purpose',
                                                 'Missing_TicketType', 'Available_TicketType'])['T_Demand'].sum().reset_index()
    no_factors_df = no_factors_df.groupby(['ZoneCodeFrom', 'ZoneCodeTo'])['T_Demand'].sum().reset_index()
    
    return other_tickets_df, no_factors_df, no_factor_demand_prop, tickets_demand_prop, tickets_internal_prop, factors_internal_prop
...
def SumPeriodsDemand(am, ip, pm, op):
    ...
    comb = comb.fillna(0)
    #sum 24Hr demand
    comb['Demand'] = comb['AM_Demand'] + comb['IP_Demand'] + comb['PM_Demand'] + comb['OP_Demand']
    #keep needed columns
...
def FromTo2FromByAveraging(matrices_dict):
    ...
    matrices = {}
    #HBEBCA_Int
    mx1 = AverageTwoMatrices(matrices_dict['HBEBCA_Int'], TransposeMatrix(matrices_dict['HBEBCA_Int_T']))
    matrices['HBEBCA_Int'] = mx1
    #HBEBNCA_Int
    mx2 = AverageTwoMatrices(matrices_dict['HBEBNCA_Int'], TransposeMatrix(matrices_dict['HBEBNCA_Int_T']))
    ...
    #HBWCA_Int
    mx5 = AverageTwoMatrices(matrices_dict['HBWCA_Int'], TransposeMatrix(matrices_dict['HBWCA_Int_T']))
    matrices['HBWCA_Int'] = mx5
    #HBWNCA_Int
    mx6 = AverageTwoMatrices(matrices_dict['HBWNCA_Int'], TransposeMatrix(matrices_dict['HBWNCA_Int_T']))
    matrices['HBWNCA_Int'] = mx6
    #HBOCA_Int
    mx7 = AverageTwoMatrices(matrices_dict['HBOCA_Int'], TransposeMatrix(matrices_dict['HBOCA_Int_T']))
    matrices['HBOCA_Int'] = mx7
    #HBONCA_Int
    mx8 = AverageTwoMatrices(matrices_dict['HBONCA_Int'], TransposeMatrix(matrices_dict['HBONCA_Int_T']))
...
def RunEDGEGrowth(params):
    ...
    logging.basicConfig(filename = f'{params.output_path}/EDGE_Factoring_{params.forecast_year}.Log', format = '%(levelname)s:%(message)s', level = logging.INFO)
    logging.info("######################################################################################")
    ...
    norms_segments = ['HBEBCA_Int', 'HBEBNCA_Int', 'NHBEBCA_Int', 'NHBEBNCA_Int', 'HBWCA_Int',
    ...
    internal_to_home = ['HBEBCA_Int_T', 'HBEBNCA_Int_T', 'NHBEBCA_Int_T', 'NHBEBNCA_Int_T',
                        'HBWCA_Int_T', 'HBWNCA_Int_T', 'HBOCA_Int_T', 'HBONCA_Int_T',
                        'NHBOCA_Int_T', 'NHBONCA_Int_T']
    ...
    segments_method = {'HBEBCA_Int':1, 'HBEBNCA_Int':1, 'NHBEBCA_Int':2, 'NHBEBNCA_Int':2,
                        'HBWCA_Int':1, 'HBWNCA_Int':2, 'HBOCA_Int':1, 'HBONCA_Int':2,
                        'NHBOCA_Int':2, 'NHBONCA_Int':2, 'HBEBCA_Int_T':1, 'HBEBNCA_Int_T':1, 
                        'NHBEBCA_Int_T':2, 'NHBEBNCA_Int_T':2, 'HBWCA_Int_T':1, 'HBWNCA_Int_T':2,
                        'HBOCA_Int_T':1, 'HBONCA_Int_T':2, 'NHBOCA_Int_T':2, 'NHBONCA_Int_T':2,
                        'EBCA_Ext_FM':1, 'EBNCA_Ext':2, 'HBWCA_Ext_FM':1, 'HBWNCA_Ext':2,
                        'OCA_Ext_FM':1, 'ONCA_Ext':2, 'EBCA_Ext_TO':1, 'HBWCA_Ext_TO':1,
                        'OCA_Ext_TO':1}
    ...
    for period in tqdm(periods, desc='Time Periods Loop ', unit= ' Period', colour='cyan'):
        logging.info(f'-- Processing Time Period {period} @ {datetime.now().strftime("%d-%m-%Y,,,%H:%M:%S.%f")}')
        ...
        iRSjProps = pd.read_hdf(f'{params.base_mx_files}/{period}_iRSj_probabilities.h5', key='iRSj')
        #period dictionary
        factored_matrices[period] = {}
        logging.info(f'Demand Segment           Base_Demand             {params.forecast_year}_Demand')
        #loop over demand segments
        for segment in tqdm(demand_segment_list, desc='    Demand Segments Loop ', unit= ' Segment', colour='cyan'):
            #demand matrices
            ...
            demandMX['userclass'] =  segments_Lookup[segments_Lookup['MX'] == segment].reset_index().iloc[0]['userclass']
            #keep needed columns
            demandMX = demandMX[['from_model_zone_id', 'to_model_zone_id', 'userclass', 'Demand']]
            ...
            #assign EDGE flows
            demandMX = AssignEDGEFlow(edge_flows_file,
                                        flow_cats, 
                                        demandMX)
            #add TAG flows
            ...
            demandMX = demandMX.merge(ticket_flow_props, how='left', on=['TAG_Flow', 'Purpose'])
            #apply Ticket Splits
            demandMX = ApplyTicketSplits(demandMX)
            #Get factors for missing movements if any
            factors_EDGE, other_tickets_df, no_factors_df = CreateFactorsForMissingMOIRAMovements(demandMX, factors_EDGE,
                                                                        other_tickets_df, no_factors_df)
            ...
            demandMX = demandMX.groupby(['from_model_zone_id', 'to_model_zone_id'])[['T_Demand', 'N_Demand']].sum().reset_index()
            tot_output_demand = round(demandMX['N_Demand'].sum())
            logging.info(f'{segment}                 {tot_input_demand}                   {tot_output_demand}')
    ...
    #get logging stats
    other_tickets_df, no_factors_df, no_factor_demand_prop, tickets_demand_prop, tickets_internal_prop, factors_internal_prop = Prepare_logging_info(other_tickets_df, no_factors_df, demand_total)  
    ...
    if no_factor_demand_prop > 1:
        logging.warning(f'          Demand with no factors  = {no_factor_demand_prop}% exceeding the 1% threshold of the total demand hence the process terminated')
    ...
    #       types have been used as well as movements with no factors at all
        ...
        logging.info('          Records below have missing factors for -Missing_TicketType- and therefore growth factors for')
        logging.info('          Tickets from Availabkle_TicektType- have been used')
        logging.info(f'          Total demand proportion for these movements = {tickets_demand_prop}% of which {tickets_internal_prop}% is Internal')
        logging.info('          -----------------------------------')
        logging.info('{}'.format(other_tickets_df.to_string(index=False)))
        #log info
        logging.warning('          Records below have no factors at all for these movements hence no growth have been applied:')
        logging.warning(f'          Total demand proportion for these movements = {no_factor_demand_prop}% of which {factors_internal_prop}% is Internal')
        logging.warning('          -----------------------------------')
        logging.warning('{}'.format(no_factors_df.to_string(index=False)))
