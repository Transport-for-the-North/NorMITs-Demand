...
class SectorReporter:
    ...
    def __init__(
        ...
        self.default_zone_system = pd.read_csv(default_zone_file)[["model_zone_id"]]
        self.default_sector_grouping = pd.read_csv(default_sector_grouping_file)[
            ["grouping_id", "model_zone_id"]
        ]

    def calculate_sector_totals(
        self,
        ...
        sector_totals = pd.DataFrame(columns=grouping_dataframe_columns)

        for group in groupings:
            zones = sector_grouping[sector_grouping["grouping_id"] == group][
                "model_zone_id"
            ].values
            calculating_dataframe_mask = calculating_dataframe["model_zone_id"].isin(
                zones
            )
    ...
    def calculate_sector_totals_v2(
        self,
        calculating_dataframe: pd.DataFrame,
        ...
        sector_grouping.rename({zone_id_column: "model_zone_id"}, axis=1, inplace=True)
        res = calculating_dataframe.merge(sector_grouping, on="model_zone_id")
