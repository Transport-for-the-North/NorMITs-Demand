...
class PopEmpComparator:
    ...
    def __init__(
        ...
        if str(self.base_year) not in self.years:
            raise ValueError(
                f"Base year ({self.base_year}) not found in the "
    ...
    def compare_totals(self) -> pd.DataFrame:
        ...
        totals = pd.DataFrame(
            {
                "input total": self.input_data[self.base_year].sum(),
        ...
        totals["constraint difference"] = (
            totals["output total"] - totals["constraint total"]
        )
        totals["constraint % difference"] = (
            totals["output total"] / totals["constraint total"]
        ) - 1
        totals["output growth"] = (
            totals["output total"] / totals.loc[self.base_year, "output total"]
        )
        totals["growth difference"] = (
            totals["output growth"] - totals["mean growth input"]
        )
        return totals
    ...
    def _compare_dataframes(
        ...
        for yr in self.years:
            comp[(yr, "constraint difference")] = (
                comp[(yr, "output")] - comp[(yr, "constraint")]
            )
            comp[(yr, "constraint % difference")] = (
                comp[(yr, "output")] / comp[(yr, "constraint")]
            ) - 1
            comp[(yr, "output growth")] = (
                comp[(yr, "output")] / comp[(self.base_year, "output")]
            )
            comp[(yr, "growth difference")] = (
                comp[(yr, "output growth")] - comp[(yr, "growth")]
            )
        ...
        for (nm, df), met_cols in zip(
            ...
            df = df.rename(
                columns={
                    ...
                    + "_id": self.sector_params["sector_system_name"]
                }
            )
            sector_data[nm] = df[
                [self.sector_params["sector_system_name"], SPLIT_COL, *original_cols]
            ]
        ...
        for (nm, df), agg in zip(sector_data.items(), 3 * ["sum"] + ["mean"]):
            if agg == "sum":
                ...
                weighted_avg = lambda x: pd.Series(
                    np.average(x[metric_cols[nm]], axis=0, weights=x[SPLIT_COL]),
                    metric_cols[nm],
                )
    ...
    def write_comparisons(
        self, output_loc: str, output_as: str = "excel", year_col: bool = False
    ):
        ...
        if output_as not in accepted_values:
            raise ValueError(
                f'output_as should be one of {accepted_values} not "{output_as}"'
            )
        ...
        comparisons = (
            (self.compare_totals, ("Totals Comparison",)),
        ...
        for func, names in comparisons:
            ...
            for nm, df in zip(names, dataframes):
                ...
                if (
                    output_as == "csv"
                    or len(df) > EXCEL_MAX[0]
                    or len(df.columns) > EXCEL_MAX[1]
                ):
                    du.safe_dataframe_to_csv(
                        df, output_dir / f"{nm}.csv", flatten_header=True
                    )
            ...
            with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
                ...
                for nm, df in outputs.items():
                    ...
                    if "ratio" in nm.lower():
                        ...
                        num_format = [
                            "Percent" if not pat.match(str(c)) else "Comma [0]"
                            for c in df.columns
                        ]
                    else:
                        pat = re.compile(r".*(percent|\%|growth).*", re.IGNORECASE)
                        num_format = [
                            "Percent" if pat.match(str(c)) else "Comma [0]"
                            for c in df.columns
                        ]
                    ...
                    _excel_column_format(
                        writer.sheets[nm], num_format, len(df.columns.names)
                    )
